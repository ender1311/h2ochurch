      ...
      <div className="mt-8 grid gap-4">
        {pages.map((p) => (
          <div key={p.slug}
            ...
            <Link href={`/admin/pages/${p.slug}/edit`} className="group min-w-0 flex-1" aria-label={`Edit ${p.title}`}>
            ...
            <div className="flex items-center gap-3">
              <Link href={`/admin/pages/${p.slug}/edit`}
                className="rounded-full bg-water/10 px-3 py-1 text-xs font-bold uppercase tracking-wider text-brand hover:bg-water/20">
                Edit
              </Link>
